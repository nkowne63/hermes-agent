...
class SessionResetPolicy:
    ...
    Configured via the ``session_reset`` section in config.yaml (bridged to
    ``default_reset_policy``) or ``reset_by_type`` / ``reset_by_platform`` overrides.
    A due session is suspended by the housekeeping sweep and replaced on the next
    inbound message (or reset directly at routing time when the sweep has not run).
    """
    mode: str = "none"
...
class ChannelOverride:
    """Per-channel model/provider/system_prompt/reasoning_effort/allow_silence override (``platforms.<name>.channel_overrides[channel_id]``)."""
    ...
    # True lets a HUMAN turn in this channel end on a bare silence marker without the
    # visible ⚠️ fallback notice — for channels where the agent may legitimately stay quiet.
    allow_silence: Optional[bool] = None
...
class PlatformConfig:
    ...
    gateway_fallback_notification: bool = False  # "⚠️ Provider fallback" ping to home_channel on runtime fallback
    ...
    _TYPED_KEYS = frozenset({
        ...
        "gateway_restart_notification", "gateway_fallback_notification", "typing_indicator", "typing_status_text",
    })
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        return cls(
            ...
            gateway_fallback_notification=_coerce_bool(toplevel_or_extra("gateway_fallback_notification"), False),

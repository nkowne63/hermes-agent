...
class DiscordAdapter(BasePlatformAdapter):
            ...
            async def on_message(message: DiscordMessage):
                ...
                if not isinstance(message.channel, discord.DMChannel) and message.mentions:
                    ...
                    if _ignore_no_mention and not _self_mentioned and not _other_bots_mentioned:
                        ...
                        _category_id = adapter_self._get_category_id(message.channel)
                        _guild_id = adapter_self._get_guild_id(message)
                        _free_channels = adapter_self._discord_free_response_channels()
                        _channel_ids = {_channel_id}
                        if _parent_id:
                            _channel_ids.add(_parent_id)
                        if _category_id:
                            _channel_ids.add(_category_id)
                        _free_by_rule = (
                            adapter_self._discord_effective_require_mention(
                                channel_id=_channel_id,
                                parent_channel_id=_parent_id,
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
        ...
        if not is_thread and not isinstance(message.channel, discord.DMChannel):
            ...
            skip_thread = bool(channel_ids & no_thread_channels) or is_free_channel or not thread_response
